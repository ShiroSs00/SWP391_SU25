...
import type { ProfileData, DonationRecord, Achievement, EventParticipation } from '../types/dashboard.type';
...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const completedDonations = donationHistory.filter(d => d.status === 'Completed' || d.status === 'Success' || d.status === 'SEPARATED').length;
  const totalVolume = donationHistory
    .filter(d => d.status === 'Completed' || d.status === 'Success' || d.status === 'SEPARATED')
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-yellow-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-800">{unlockedAchievements}</p>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-purple-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-gray-800">{upcomingEvents}</p>
          ...
          {recentDonations.length === 0 ? (
            ...
            <div className="space-y-4">
              {recentDonations.map((donation) => (
                <div key={donation.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        donation.status === 'Completed' || donation.status === 'Success' || donation.status === 'SEPARATED'
                          ? 'bg-green-100 text-green-800'
                          : donation.status === 'Pending'
                          ? 'bg-yellow-100 text-yellow-800'
                          : 'bg-red-100 text-red-800'
                      }`}>
                        {donation.status === 'Completed' || donation.status === 'Success' || donation.status === 'SEPARATED' ? 'Hoàn thành' : 
                         donation.status === 'Pending' ? 'Đang chờ' : 'Đã hủy'}
                      </span>
                    </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <span className="text-blue-600 text-sm">
                {recentDonations.length > 0 ? formatDate(recentDonations[0].date) : 'Chưa có'}
              </span>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-purple-50 rounded-lg border border-purple-200">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium border ${getBloodTypeColor(profile.bloodType)}`}>
                {profile.bloodType}
              </span>
            </div>
          </div>
            ...
            <div className="flex items-start space-x-3 p-4 bg-orange-50 rounded-lg border border-orange-200">
              ...
              <div>
                ...
                <p className="text-sm text-orange-600 mt-1">
                  {profile.emergencyNotifications ? 'Đã bật - Bạn sẽ nhận thông báo khi có yêu cầu khẩn cấp' : 'Đã tắt'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div>
                ...
                <p className="text-sm text-blue-600 mt-1">
                  Lần hiến tiếp theo có thể sau {formatDate(new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString())}
                </p>
              </div>
            </div>
