...
import type { BlogFilters, BlogCategory, BlogListProps } from '../types/blog.types';

const categoryOptions = [
    { value: 'blood_education', label: 'Giao duc ve mau'},
    { value: 'donation_tips', label: 'Meo hien mau'},
    { value: 'health_wellness', label: 'Suc khoe'},
    { value: 'success_stories', label: 'Cau chuyen thanh cong'},
    { value: 'mediacal_research', label: 'Nghien cuu y khoa'},
    { value: 'community', label: 'Cong dong'},
    { value: 'news', label: 'Tin tuc'},
    { value: 'events', label: 'Su kien'},
];
...
const { posts, loading, error, pagination, fetchPosts } =
useBlog();
...
const handleFilterChange = (newFilters: Partial<BlogFilters>) => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Posts Grid */}
            {/*
                Extracted grid content into a variable to avoid nested ternary
            */}
            {(() => {
                let gridContent;
                if (loading && posts.length === 0) {
                    gridContent = Array.from({ length: 6 }).map((_, index) => (
                    ...
                    gridContent = filteredPosts.map(post => (
                        <BlogCard
                            key={post.id}
                            post={post}
