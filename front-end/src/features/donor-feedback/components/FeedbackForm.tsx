...
const FeedbackForm: React.FC<FeedbackFormProps> = ({
  ...
  // Initialize form data từ initialData
  useEffect(() => {
    console.log('FeedbackForm rendered with:', { registrationId, initialData }); // Debug log
    
    if (initialData) {
      console.log('Setting form data from initialData:', initialData); // Debug log
      setFormData({
      ...
      // Nếu có initialData, có nghĩa là đang edit feedback existing
      setCanSubmit(true);
      setExistingFeedback(null);
    } else if (registrationId) {
      // Chỉ check eligibility khi không có initialData
      checkFeedbackEligibility();
    }
  }, [registrationId]); // Chỉ depend vào registrationId, không depend vào initialData để tránh loop

  // Separate useEffect for initialData changes
  useEffect(() => {
    if (initialData) {
      console.log('Updating form data from initialData:', initialData); // Debug log
      setFormData({
        process: initialData.process || 0,
        bloodTest: initialData.bloodTest || 0,
        postDonationCare: initialData.postDonationCare || 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Submitting feedback form:', formData); // Debug log
    ...
    try {
      ...
      console.error('Error submittin  g feedback:', error); // Debug log
      const errorMessage = formatErrorMessage(error);
      toast.error(errorMessage);
    }
  };
  ...
  // Show existing feedback (chỉ khi không có initialData - tức là không phải edit mode)
  if (existingFeedback && !initialData) {
  ...
  if (canSubmit === false && !existingFeedback && !initialData) {
    return (
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
  ...
  // Main feedback form - chỉ hiện khi canSubmit = true hoặc có initialData
  if (canSubmit === true || initialData) {
    return (
          ...
          <div className="bg-gradient-to-r from-red-500 to-pink-600 px-8 py-8">
            <div className="text-center text-white">
              ...
              <h2 className="text-3xl font-bold mb-2">
                {initialData ? 'Chỉnh sửa đánh giá' : 'Đánh giá trải nghiệm hiến máu'}
              </h2>
              <p className="text-red-100 text-lg">
                {initialData ? 'Cập nhật đánh giá trải nghiệm hiến máu của bạn' : 'Chia sẻ trải nghiệm của bạn để giúp chúng tôi cải thiện dịch vụ'}
              </p>
            </div>
          </div>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    {initialData ? 'Cập nhật feedback' : 'Gửi feedback'}
                  </>
                )}
              </button>
            </div>
