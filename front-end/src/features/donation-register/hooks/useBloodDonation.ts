...
export const createDonation = async (id: string, data: DonationCreatePayload): Promise<DonationRegistrationDTO> => {
  const res = await api.post<DonationRegistrationDTO>(`/donation/create/${id}`, data);
  return res.data;
};
